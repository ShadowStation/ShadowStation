...
/client/verb/linkdiscord()
	...
	if(!SSdiscord.enabled)
		to_chat(src, "<span class='warning'>TGS is not running or is not connected.</span>")
		return
	...
	if(!stored_id) // Account is not linked
		...
		alert(usr, "Account link started. Please ping the bot of the server you\'re currently on, follows by \"verify [usr.ckey]\" in the Discord to successfuly verify your account (Example: @ShadowBot verify [usr.ckey])")
