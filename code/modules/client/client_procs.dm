...
/client/proc/set_client_age_from_db(connectiontopic)
	...
	if(!query_client_in_db.NextRow())
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
			...
			to_chat(src, "<span class='notice'>You must first join the Discord to verify your account before joining this server.<br>To do so, read the rules and post a request in the #access-requests channel under the \"Server function\" category in the Discord server linked here: <a href='https://discord.gg/vD7AAW5'>https://discord.gg/vD7AAW5</a></span>") //CIT CHANGE - makes the panic bunker disconnect message point to the discord
